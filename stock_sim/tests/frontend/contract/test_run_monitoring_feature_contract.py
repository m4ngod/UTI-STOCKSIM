...
def test_wave_three_entry_activates_strategy_library_before_inherited_interfaces():
    ...
    assert (
        ACTIVE_FEATURE_INTERFACES[2].version == DIAGNOSTIC_TASKS_INTERFACE_VERSION
    )
    assert (
        ACTIVE_FEATURE_INTERFACES[3].version == RUN_MONITORING_INTERFACE_VERSION
    )
    assert (
...
def test_fake_disconnected_without_prior_data_is_failed_not_empty():
    adapter = DeterministicFakeRunMonitoringAdapter()
    context = RunMonitoringContext.no_selection()
