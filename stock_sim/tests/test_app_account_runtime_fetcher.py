...
def test_app_account_service_prefers_runtime_fetcher_after_runtime_trade():
    symbol = f"AF{uuid.uuid4().hex[:6].upper()}"
    ...
    try:
        ...
        dto = AppAccountService().load_account(buyer_id)
        symbols = {p.symbol for p in dto.positions}
