...
def test_ipo_open_trade_should_update_buyer_runtime_position_via_settlement_bridge():
    symbol = f"IPOSB{uuid.uuid4().hex[:6].upper()}"
    ...
    try:
        ...
        engine = MatchingEngine(
            symbol,
            create_instrument(symbol, tick_size=0.01, lot_size=100, min_qty=100, initial_price=15.0),
        )
        ...
        assert buyer_pos.quantity >= 100_000
    finally:
        s.close()
