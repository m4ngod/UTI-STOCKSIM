...
- [x] Added `ArenaExperimentRunner` as the repeatable experiment orchestration entry point.
- [x] The runner can create an Arena, start/stop the runtime clock, run an episode window, evaluate rankings, optionally trigger PBT, and write a JSON experiment report.
- [x] Added `scripts/run_arena_experiment.py` for command-line Arena experiments.
- [x] Registered the runner in `AppContext` for future desktop-panel control.
- [x] Added regression coverage for runner orchestration and PBT report generation.

### Completed in round 14

- [x] Wired `ArenaExperimentRunner` into the desktop Arena panel logic.
- [x] Added a non-blocking `Run Experiment` control to the Arena UI adapter.
- [x] Arena view now exposes the latest experiment report path, transition count, result count, PBT checkpoint count, and lineage count.
- [x] Added frontend unit coverage for experiment execution and report-summary rendering.

### Completed in round 15

- [x] Split submitted order intent from filled turnover in `RewardBuilder` so unfilled model orders no longer receive filled-turnover penalties.
- [x] Added execution-health metrics to episode results and Arena experiment reports: submitted/open/rejected/filled order counts, notional fill ratio, and filled notional.
- [x] Added Arena training liquidity seeding through dedicated BID/ASK liquidity accounts so model orders can fill through the normal runtime order book.
- [x] Fixed first-step reward baselining so initial account cash is not treated as model profit.
- [x] Made model marketable-limit pricing read live order-book levels even before throttled snapshots refresh.
- [x] Restricted Arena-runner PBT inheritance to eligible trainable models by default; `hold_model_v1` and `random_weight_v1` remain report baselines but are excluded from parent selection.
- [x] Verified the loop with command-line Arena runs: PPO/LSTM generated persisted transitions, real fills, execution-health report data, and a PBT checkpoint without baseline inheritance.

### Completed in round 16

- [x] Changed the default Arena experiment population to three independently seeded `ppo_lstm_v1` online-training agents plus hold/random baselines.
- [x] Added a PBT parent activity gate so inherited parents must have real filled activity; inactive trainable models can no longer become parents by merely avoiding trading costs.
- [x] Increased default Arena training liquidity depth to support multiple PPO agents in one episode.
- [x] Added CLI/report fields for PBT parent eligibility: `parent_eligible_agents` and `parent_activity_gate`.
- [x] Verified a 90-second multi-PPO run: all three PPO agents produced fills, PBT selected an active PPO parent, and lineage was created for the weakest PPO while baselines stayed excluded.

### Completed in round 17

- [x] Added sequential multi-generation support to `ArenaExperimentRunner`.
- [x] Added `--generations` to `scripts/run_arena_experiment.py` so command-line runs can execute multiple Arena generations in one series.
- [x] Added generation-series JSON reports with aggregate transitions, trades, fill ratios, checkpoint counts, lineage counts, winner/loser counts, and final model ids.
- [x] Made PBT lineage child model ids carry into the next generation's model specs, even when persisted agent state is stale.
- [x] Made PBT applied-agent report rows use the explicit `child_model_id` produced by lineage.
- [x] Verified a two-generation PPO/LSTM smoke run where a generation-0 loser entered generation 1 as `ppo_lstm_v1.gen1.*`, produced fills/transitions, and remained eligible for later PBT.

### Not done yet

No extra feature checklist is being added here. The next work should use the implemented multi-generation runner for longer series, then tune reward weights and liquidity depth from the reports.

## 1. 文档目的

本文件用于规划 UTI-STOCKSIM 下一阶段的主线工作：从“桌面级交易仿真平台”改造为“多智能体对抗交易训练平台”。

上一份文档 `docs/design/model-training-design.md` 主要回答：
...
- `README.md`：项目总览
- `docs/design/model-training-design.md`：训练范式和算法设计
- `docs/plan/multi-agent-training-roadmap.md`：下一阶段工程路线
- `docs/contracts/runtime/model-observation-contract.md`
- `docs/contracts/runtime/model-action-contract.md`
