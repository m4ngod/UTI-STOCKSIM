...
- **change summary**:
  ...
  - 对该测试的进一步定位确认了两个事实：
    ...
    5. 随后新增 `tests/test_app_account_runtime_fetcher.py` 并通过，确认 `app/services/account_service.py` 在 runtime trade 之后能够优先读到本地 runtime DB，而不是退回 synthetic 数据。
    6. 再结合 `scripts/debug_account_panel_visibility.py` 的结果，已经确认 `AccountPanel` 在独立验证路径中也能正确显示 runtime 持仓。
    7. 因而 `tests/test_release_minimal_runtime_chain.py` 当前未绿，更应视为 **测试构造/时序耦合噪声**，而不是当前产品链路的主 blocker。当前产品层面的关键链路（IPO settlement + app-layer account fetch + account panel visibility）已分别得到独立验证。
