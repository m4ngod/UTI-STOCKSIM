# Arena Missing Sources Completeness Status
...
Arena experiment metadata already records `experiment_record_metadata.missing_sources`. This task makes the missing-sources list visible in `experiment_record_completeness` so reports can distinguish an explicit missing-source ledger from a report that lacks such a ledger.

This is a status mapping only. It does not resolve missing sources.

## Scope

Implemented:

- Add `missing_sources` to `experiment_record_completeness.field_status`.
- Mark `missing_sources=present` when a top-level or metadata missing-sources list exists.
- Mark `missing_sources=missing` when no missing-sources list exists.
- Carry the field into `aggregate.experiment_record_completeness.field_status_counts`.

## Explicitly Not Implemented

- No missing-source remediation.
